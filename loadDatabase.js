...
let Photo = require("./schema/photo.js");
let SchemaInfo = require("./schema/schemaInfo.js");
...
Promise.all(removePromises)
  .then(function () {
    ...
    let userModels = cs142models.userListModel();
    let mapFakeId2RealId = {};
    let userPromises = userModels.map(function (user) {
      return User.create({
    ...
    let allPromises = Promise.all(userPromises).then(function () {
      ...
      let photoModels = [];
      let userIDs = Object.keys(mapFakeId2RealId);
      for (let i = 0; i < userIDs.length; i++) {
        photoModels = photoModels.concat(
          cs142models.photoOfUserModel(userIDs[i])
        );
      }
      let photoPromises = photoModels.map(function (photo) {
        return Photo.create({
