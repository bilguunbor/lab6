...
let cs142models = require("../modelData/photoApp.js").cs142models;

let port = 3000;
let host = "localhost";

// Valid properties of a user list model
let userListProperties = ["first_name", "last_name", "_id"];
// Valid properties of a user detail model
let userDetailProperties = [
  "first_name",
  "last_name",
  "_id",
  "location",
...
let photoProperties = ["file_name", "date_time", "user_id", "_id"];
// Valid comments properties
let commentProperties = ["comment", "date_time", "_id", "user"];
...
describe("CS142 Photo App API - ", function () {
  describe("test using model data", function (done) {
    it("webServer does not use model data", function (done) {
      fs.readFile("../webServer.js", function (err, data) {
        if (err) throw err;
        let regex = /\n\s*let cs142models = require\('\.\/modelData\/photoApp\.js'\)\.cs142models;/g;
        assert(
  ...
  describe("test /user/list", function (done) {
    let userList;
    let cs142Users = cs142models.userListModel();
          ...
          http.get(
            ...
            function (response) {
              ...
              response.on("end", function () {
                ...
                let real_photos = cs142models.photoOfUserModel(realUser._id);
                ...
                _.forEach(real_photos, function (real_photo) {
                  ...
                  let photo = matches[0];
                  let photoProps = photoProperties;
                  if (true) {
                    photoProps = photoProps.concat("comments");
                  }
                  let extraProps1 = _.difference(
                  ...
                  if (real_photo.comments) {
                    ...
                    _.forEach(real_photo.comments, function (real_comment) {
                      let comment = _.find(photo.comments, {
                        comment: real_comment.comment,
                      });
                      assert(comment);
                      let extraProps2 = _.difference(
